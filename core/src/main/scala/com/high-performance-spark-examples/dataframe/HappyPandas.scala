...
object HappyPandas {
  ...
  def hiveContext(sc: SparkContext): SQLContext = {
    ...
    val hiveContext =
      SparkSession.builder.enableHiveSupport().getOrCreate().sqlContext
    // Import the implicits, unlike in core Spark the implicits are defined
    // on the context.
  ...
  /** @param place
    *   name of place
    * @param pandaType
    *   type of pandas in this place
    * @param happyPandas
    *   number of happy pandas in this place
    * @param totalPandas
    *   total number of pandas in this place
    */
  ...
  /** Gets the percentage of happy pandas per place.
    ...
    *   the input DataFrame
    * @return
    *   Returns DataFrame of (place, percentage of happy pandas)
    */
  ...
  /** Encodes pandaType to Integer values instead of String values.
    ...
    *   the input DataFrame
    * @return
    *   Returns a DataFrame of pandaId and integer value for pandaType.
    */
  ...
  /** Extra the panda info from panda places and compute the squisheness of the
    * panda
    */
  ...
  def minMeanSizePerZip(pandas: DataFrame): DataFrame = {
    ...
    pandas
      .groupBy(pandas("zip"))
      .agg(min(pandas("pandaSize")), mean(pandas("pandaSize")))
  }
  ...
  /** Orders pandas by size ascending and by age descending. Pandas will be
    * sorted by "size" first and if two pandas have the same "size" will be
    * sorted by "age".
    */
  ...
  def badJoin(
    ...
    val sle = session.udf.register(
      "strLenEq",
      (s: String, s2: String) => s.length() == s2.length()
    )
