...
object GenerateScalingData {
  ...
  /** Generate a Goldilocks data set. We expect the zip code to follow an
    * exponential distribution and the data its self to be normal
    *
    ...
    * @param rows
    *   number of rows in the RDD (approximate)
    * @param size
    *   number of value elements
    */
  def generateFullGoldilocks(
      sc: SparkContext,
      rows: Long,
      numCols: Int
  ): RDD[RawPanda] = {
    val zipRDD = RandomRDDs
      .exponentialRDD(sc, mean = 1000, size = rows)
      .map(_.toInt.toString)
    val valuesRDD = RandomRDDs
      .normalVectorRDD(sc, numRows = rows, numCols = numCols)
      .repartition(zipRDD.partitions.size)
    val keyRDD = sc.parallelize(1L.to(rows), zipRDD.getNumPartitions)
    keyRDD
      .zipPartitions(zipRDD, valuesRDD) { (i1, i2, i3) =>
  ...
  /** Transform it down to just the data used for the benchmark
    */
  def generateMiniScaleRows(
      sc: SparkContext,
      rows: Long,
      numCols: Int
  ): RDD[Row] = {
    generateMiniScale(sc, rows, numCols).map { case (zip, fuzzy) =>
      Row(zip, fuzzy)
    }
  }

  // tag::MAGIC_PANDA[]
  /** Generate a Goldilocks data set all with the same id. We expect the zip
    * code to follow an exponential distribution and the data its self to be
    * normal. Simplified to avoid a 3-way zip.
    *
    * Note: May generate less than number of requested rows due to different
    * distribution between partitions and zip being computed per partition.
    */
  def generateGoldilocks(
      sc: SparkContext,
      rows: Long,
      numCols: Int
  ): RDD[RawPanda] = {
