...
class SmartAggregations {
  ...
  /** Given an RDD of (PandaInstructor, ReportCardText) aggregate by instructor
    ...
    * instructor happyMentions -> The number of times this intructor mentioned
  ...
  def calculateReportCardStatistics(
    ...
    rdd
      .aggregateByKey(
        ...
        seqOp = (
            (reportCardMetrics, reportCardText) =>
  ...
  /** Same as above, but rather than using the 'MetricsCalculator' class for
    ...
    * @return
    */
  def calculateReportCardStatisticsReuseObjects(
      rdd: RDD[(String, String)]
  ): RDD[(String, ReportCardMetrics)] = {

    rdd
      .aggregateByKey(
        new MetricsCalculatorReuseObjects(
          totalWords = 0,
          longestWord = 0,
          happyMentions = 0,
          numberReportCards = 0
  ...
  // tag::goodAggregation[]
  def calculateReportCardStatisticsWithArrays(
      rdd: RDD[(String, String)]
  ): RDD[(String, ReportCardMetrics)] = {
      ...
      .aggregateByKey(
        ...
        seqOp = (reportCardMetrics, reportCardText) =>
          MetricsCalculator_Arrays
            .sequenceOp(reportCardMetrics, reportCardText),
...
object MetricsCalculator_Arrays extends Serializable {
  ...
  def sequenceOp(
    ...
    reportCardMetrics(happyMentionsIndex) += words.count(w =>
      w.toLowerCase.equals("happy")
    )
