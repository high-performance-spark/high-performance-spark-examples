...
object PandaSecondarySort {
  ...
  /** Sort first by panda Id (a tuple of four things) Name, address, zip,
    * happiness, Then by city, zip, and name.
    *
    * @param rdd
    * @return
    */
  def secondarySort(rdd: RDD[(String, StreetAddress, Int, Double)]) = {
    val keyedRDD: RDD[(PandaKey, (String, StreetAddress, Int, Double))] =
      rdd.map { case (fullName, address, zip, happiness) =>
        (
  ...
  def groupByCityAndSortWithinGroups(
    ...
    val sortedOnPartitions
        : RDD[(PandaKey, (String, StreetAddress, Int, Double))] =
      keyedRDD.repartitionAndSortWithinPartitions(pandaPartitioner)
...
object SecondarySort {
  ...
  // tag::sortAndGroup[]
  def groupByKeyAndSortBySecondaryKey[
      K: Ordering: ClassTag,
      S: Ordering: ClassTag,
      V: ClassTag
