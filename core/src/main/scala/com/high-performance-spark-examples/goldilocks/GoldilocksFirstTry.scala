...
object GoldilocksGroupByKey {
  ...
  def findRankStatistics(
    ...
    groupColumns
      .mapValues(iter => {
        ...
        sortedIter.toIterable.zipWithIndex.flatMap({ case (colValue, index) =>
  ...
  def mapToKeyValuePairs(dataFrame: DataFrame): RDD[(Int, Double)] = {
    ...
    dataFrame.rdd.flatMap(row =>
      Range(0, rowLength).map(i => (i, row.getDouble(i)))
    )
  }
  // end::toKeyValPairs[]
}
...
object GoldilocksFirstTry {
  ...
  /** Step 1. Map the rows to pairs of (value, column Index).
    ...
    * For example:
    *
    * dataFrame: 1.5, 1.25, 2.0 5.25, 2.5, 1.5
    *
    * The output RDD will be: (1.5, 0) (1.25, 1) (2.0, 2) (5.25, 0) (2.5, 1)
    * (1.5, 2)
    *
    * @param dataFrame
    *   dateframe of doubles
    *
    * @return
    *   RDD of pairs (value, column Index)
    */
  // tag::firstTry_Step1[]
  private def getValueColumnPairs(dataFrame: DataFrame): RDD[(Double, Int)] = {
    dataFrame.rdd.flatMap { row: Row =>
  ...
  /** Step 3: For each Partition determine the index of the elements that are
    ...
    * For Example:
    *
    * targetRanks: 5 partitionColumnsFreq: [(0, [2, 3]), (1, [4, 1]), (2, [5,
    * 2])] numOfColumns: 2
    *
    * The output will be:
    *
