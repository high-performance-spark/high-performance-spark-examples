...
object GoldilocksSecondarySort {
  ...
  /** Given an array of (columnIndex, value) pairs that are already sorted.
    * Groups the pairs with the same column index, creating an iterator of
    * values.
    */
  ...
  private def groupSorted(
    ...
    ).map { case (key, buf) => (key, buf.toIterable) }
      .toMap
  }
...
object GoldilocksSecondarySortV2 {
  ...
  def findRankStatistics(
    ...
    val partitioner = new ColumnIndexPartition(partitions)
    val sorted =
      pairRDD.map((_, 1)).repartitionAndSortWithinPartitions(partitioner)
    val filterForTargetIndex = sorted.keys.mapPartitions(
      iter => {
  ...
  def filterAndGroupRanks(
    ...
    ).map { case (key, total, buf) => (key, buf.toIterable) }
      .iterator
  }
