...
object GoldilocksWithHashMap {
  ...
  /** Find nth target rank for every column.
    ...
    * For example:
    *
    * dataframe: (0.0, 4.5, 7.7, 5.0) (1.0, 5.5, 6.7, 6.0) (2.0, 5.5, 1.5, 7.0)
    * (3.0, 5.5, 0.5, 7.0) (4.0, 5.5, 0.5, 8.0)
    *
    * targetRanks: 1, 3
    *
    * The output will be: 0 -> (0.0, 2.0) 1 -> (4.5, 5.5) 2 -> (7.7, 1.5) 3 ->
    * (5.0, 7.0)
    *
    * @param dataFrame
    *   dataframe of doubles
    * @param targetRanks
    *   the required ranks for every column
    *
    * @return
    *   map of (column index, list of target ranks)
    */
  // tag::hashMap[]
  def findRankStatistics(
    ...
    val sortedAggregatedValueColumnPairs =
      aggregatedValueColumnPairs.sortByKey()
  ...
  /** Step 1. Map the rows to pairs of ((value, colIndex), count) where count is
    * the number of times that value and that pair appear on this partition.
    *
    * For example:
    *
    * dataFrame: 1.5, 1.25, 2.0 1.5, 2.5, 2.0
    *
    * The output RDD will be: ((1.5, 0), 2) ((1.25, 1), 1) ((2.5, 1), 1) ((2.0,
    * 2), 2)
    *
    * @param dataFrame
    *   of double columns to compute the rank statistics for
    *
    * @return
    *   returns RDD of ((value, column index), count)
    */
  // tag::hashMap_step1[]
  def getAggregatedValueColumnPairs(
      dataFrame: DataFrame
  ): RDD[((Double, Int), Long)] = {
  ...
  /** Step 2. Find the number of elements for each column in each partition.
    ...
    * For Example:
    *
    * sortedValueColumnPairs: Partition 1: ((1.5, 0), 2) ((2.0, 0), 1) Partition
    * 2: ((4.0, 0), 3) ((3.0, 1), 1)
    *
    * numOfColumns: 3
    *
    * The output will be: [(0, [3, 0]), (1, [3, 1])]
    *
    * @param sortedAggregatedValueColumnPairs
    *   sortedAggregatedValueColumnPairs RDD of ((value, column index), count)
    * @param numOfColumns
    *   the number of columns
    *
    * @return
    *   Array that contains (partition index, number of elements from every
    *   column on this partition)
    */
  // tag::hashMap_step2[]
  private def getColumnsFreqPerPartition(
    ...
    def aggregateColumnFrequencies(
        partitionIndex: Int,
        pairs: Iterator[((Double, Int), Long)]
    ) = {
  ...
  /** Step 3: For each Partition determine the index of the elements that are
    ...
    *
    * For Example: targetRanks: 5 partitionColumnsFreq: [(0, [2, 3]), (1, [4,
    * 1]), (2, [5, 2])] numOfColumns: 2
    *
    * The output will be: [(0, []), (1, [(0, 3)]), (2, [(1, 1)])]
    *
    * @param partitionColumnsFreq
    *   Array of (partition index, columns frequencies per this partition)
    *
    * @return
    *   Array that contains (partition index, relevantIndexList) Where
    ...
    partitionColumnsFreq.sortBy(_._1).map {
      case (partitionIndex, columnsFreq) =>
        ...
        columnsFreq.zipWithIndex.foreach { case (colCount, colIndex) =>
          ...
          relevantIndexList ++= ranksHere
            .map(rank => (colIndex, rank - runningTotalCol))
  ...
  private def findTargetRanksIteratively(
    ...
    sortedAggregatedValueColumnPairs.mapPartitionsWithIndex(
      (
        ...
        val targetsInThisPart: List[(Int, Long)] =
          ranksLocations(partitionIndex)._2
  ...
  def findQuantilesWithCustomStorage(
    ...
    val ranksLocations =
      getRanksLocationsWithinEachPart(targetRanks, partitionColumnsFreq, n)
...
object FindTargetsSubRoutine extends Serializable {
  ...
  def withArrayBuffer(
    ...
    runningTotals ++= columnsInThisPart
      .map(columnIndex => (columnIndex, 0L))
      .toMap
    ...
    valueColumnPairsIter.foreach { case ((value, colIndex), count) =>
  ...
  /** Same function as above but rather than building the result from an array
    * buffer we use a flatMap on the iterator to get the resulting iterator.
    */
  // tag::iterToIter[]
  def asIteratorToIteratorTransformation(
    ...
    // to one of the desired rank statistics on this partition.
    pairsWithRanksInThisPart.flatMap { case (((value, colIndex), count)) =>
